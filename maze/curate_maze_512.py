"""Select a reproducible 512-row subset of the 2,048-row maze training split.

Run on the server from the repository root with
``.venv/bin/python -m maze.curate_maze_512``. The existing 128-row evaluation
...
OUTPUT_DIR = REPO_ROOT / "maze/data/maze_17_512"
TRAIN_COUNT = 512
EVAL_COUNT = 128
TRAIN_SEED = 512
...
def main() -> None:
    ...
    for filename, row_count_key, count, seed in split_specs:
        ...
        if seed is None:
            if count != source_rows:
                raise ValueError(f"Cannot preserve {source_path}: expected {count} rows, found {source_rows}")
    ...
    metadata = {
        ...
        "selection_method": ("Seeded sampling without replacement from the parent training split; the parent evaluation split is preserved in source order"),

"""Select a reproducible 1,024-row subset of the 2,048-row maze training split.

Run on the server from the repository root with
``.venv/bin/python -m maze.curate_maze_1024``. The existing 128-row evaluation
split is preserved exactly so runs at different training sizes are comparable.
"""
...
SOURCE_DIR = REPO_ROOT / "maze/data/maze_17_2048"
OUTPUT_DIR = REPO_ROOT / "maze/data/maze_17_1024"
TRAIN_COUNT = 1_024
EVAL_COUNT = 128
TRAIN_SEED = 1_024
...
def main() -> None:
    ...
    split_specs = (
        ...
        ("test.parquet", "eval_rows", EVAL_COUNT, None),
    )
    ...
    for filename, row_count_key, count, seed in split_specs:
        ...
        if source_rows != source_metadata[row_count_key] or len(fingerprints) != source_rows:
            raise ValueError(
                f"Source row count or question uniqueness validation failed: {source_path}"
            )
        ...
        if seed is None:
            if count != source_rows:
                raise ValueError(
                    f"Cannot preserve {source_path}: expected {count} rows, found {source_rows}"
                )
            subsets[filename] = source_table
            indices = list(range(source_rows))
        else:
    ...
    for filename, _, count, _ in split_specs:
        ...
        if rows != count or len(fingerprints) != count:
            raise RuntimeError(
                f"Written subset row count or uniqueness validation failed: {output_path}"
            )
    ...
    metadata = {
        ...
        "selection_method": (
            "Seeded sampling without replacement from the parent training split; "
            "the parent evaluation split is preserved in source order"
        ),
        ...
        "decontamination": {
            "method": (
                "Inherited from checksum-verified parent splits; "
                "original train/test membership is preserved"
            ),
